# STORE.md - Holy Chip Store Strategy

_How we sell Holy Chip physical products._

## Store Structure

The Holy Chip **Store** page focuses exclusively on:

**Products** - Physical merchandise (t-shirts, mugs, hats, etc.) via Printify

**Note:** NFTs are on a separate dedicated page (nfts.html). See NFTS.md for NFT strategy.

---

## Products

### What We Sell

- **Apparel:** _[t-shirts, hoodies, hats, etc.]_
- **Drinkware:** _[mugs, water bottles, etc.]_
- **Accessories:** _[stickers, pins, posters, etc.]_
- **Other:** _[what else?]_

### Design Guidelines

- **Visual Style:** Black & white only (consistent with brand identity)
- **Character Usage:** _[Which Chips appear on products? All 14? Featured characters?]_
- **Typography:** _[Font choices for product text]_
- **Layouts:** _[Simple/bold, minimal, character-focused?]_

### Printify Integration

- **Platform:** Printify.com API
- **Product Categories:** _[Which Printify product types are we using?]_
- **Fulfillment:** _[Details about shipping, handling, etc.]_
- **Pricing Strategy:** _[Markup percentage, pricing tiers, etc.]_
- **API Integration:** _[Technical notes about API setup]_

### Product Categories

_Organize products for easy browsing:_

1. **Category 1:** _[e.g., "Featured Chips" - best sellers]_
2. **Category 2:** _[e.g., "Apparel" - all clothing]_
3. **Category 3:** _[e.g., "Accessories" - smaller items]_
4. _[Add more as needed]_

---

## Pricing Philosophy

_How we think about pricing:_

- **Products:** _[Affordable, premium, balanced?]_
- **Strategy:** _[Volume vs. margin? Community-first pricing?]_

---

## Store Voice & Messaging

_How we talk about products in the store:_

- **Product Descriptions:** _[Funny? Straightforward? In-character?]_
- **Call-to-Actions:** _[What language do we use? "Get Your Chip"? "Holy Chip Merch"?]_
- **Tone:** _[Consistent with brand - subversive, humorous, direct?]_

---

## Future Expansion

_Ideas for later:_

- _[Limited edition drops?]_
- _[Collaborations?]_
- _[Subscription boxes?]_
- _[Digital downloads (wallpapers, episodes)?]_

---

**Last Updated:** 2026-02-13 (NFTs moved to separate page)

_This strategy evolves as we grow the store and learn what resonates with our audience._
