// Printify Product Test v2 - Create Chip_1 T-shirt with brand on back
// Front: Chip_1, Back: Brand
...
async function createProduct(chipName, productType, chipImageId, brandImageId, blueprintId, printProvider, variants) {
  ...
  // Create product data with FRONT (Chip) and BACK (Brand)
  const productData = {
    title: title,
  ...
  console.log('  Creating product with front (Chip) and back (Brand)...');
...
async function testProductCreation() {
  console.log('🧪 TEST v2: Creating Chip_1 T-shirt (Front: Chip, Back: Brand)\n');

  try {
    // Step 1: Upload Chip_1 image
    console.log('📦 Step 1: Uploading Chip_1 image...');
    ...
    const product = await createProduct('Chip_1', 'tshirt', chipImage.id, brandImage.id, PRINTIFY_BLUEPRINTS.tshirt, printProvider, variants);
    ...
    console.log(`   Front: Chip_1`);
    console.log(`   Back: Brand`);
    console.log(`   View: https://printify.com/app/products/${product.id}`);

    console.log('\n✅ Check this product - if it looks good, we can create all 36!');
